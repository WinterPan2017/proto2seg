...
class LinkNet(nn.Module):
    ...
    def __init__(self, n_classes=2, pretrain=""):
        """
        ...
        if pretrain != "":
            self.load_pretrained_backbone(base, pretrain)
    ...
    def load_pretrained_backbone(self, model, ck_path):
        checkpoint = torch.load(ck_path, map_location='cpu')
        state_dict = {k[7:]: v.cpu() for k, v in checkpoint["net"].items(
        ) if k[7:] in model.state_dict().keys()}

        missing_keys, unexpected_keys = model.load_state_dict(
            state_dict, strict=False)
        logger.info("loading wegight from {}".format(ck_path))
